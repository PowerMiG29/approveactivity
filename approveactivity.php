    ...
    implements IBPEventActivity, IBPActivityExternalEventListener
{
    ...
    public static function ShowTaskForm($arTask, $userId, $userName = "")
    {
        ...
        if (!array_key_exists("ShowComment", $arTask["PARAMETERS"]) || ($arTask["PARAMETERS"]["ShowComment"] != "N")) {
            ...
            $form .=
                ...
                .($arTask["PARAMETERS"]["CommentLabelMessage"] <> '' ? $arTask["PARAMETERS"]["CommentLabelMessage"] : GetMessage(
                    "BPAA_ACT_COMMENT"
                ))
                .$required
                .':</td>'.
        ...
        // добавлена кнопка 3-го поведения, повторяет nonapprove
        $buttons =
            '<input type="submit" name="approve" value="'.($arTask["PARAMETERS"]["TaskButton1Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton1Message"] : GetMessage(
                "BPAA_ACT_BUTTON1"
            )).'"/>'.
            '<input type="submit" name="revision" value="'.($arTask["PARAMETERS"]["TaskButton3Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton3Message"] : GetMessage(
                "BPAA_ACT_BUTTON3"
            )).'"/>'.
            '<input type="submit" name="nonapprove" value="'.($arTask["PARAMETERS"]["TaskButton2Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton2Message"] : GetMessage(
                "BPAA_ACT_BUTTON2"
            )).'"/>';
        ...
        return array(
            'BUTTONS' => array(
                array(
                    ...
                    'NAME' => 'approve',
                    'VALUE' => 'Y',
                    'TEXT' => $arTask["PARAMETERS"]["TaskButton1Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton1Message"] : GetMessage(
                        "BPAA_ACT_BUTTON1"
                    ),
                ),
                array(
                    ...
                    'NAME' => 'revision',
                    'VALUE' => 'Y',
                    'TEXT' => $arTask["PARAMETERS"]["TaskButton3Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton3Message"] : GetMessage(
                        "BPAA_ACT_BUTTON3"
                    ),
                ),
                array(
                    ...
                    'NAME' => 'nonapprove',
                    'VALUE' => 'Y',
                    'TEXT' => $arTask["PARAMETERS"]["TaskButton2Message"] <> '' ? $arTask["PARAMETERS"]["TaskButton2Message"] : GetMessage(
                        "BPAA_ACT_BUTTON2"
                    ),
                ),
            ),
    ...
    public static function PostTaskForm($arTask, $userId, $arRequest, &$arErrors, $userName = "", $realUserId = null)
    {
        ...
        try {
            ...
            // добавим обработку события revision, которое повторит отмену
            ...
            } elseif (isset($arRequest['revision']) && $arRequest["revision"] <> ''
                || isset($arRequest['INLINE_USER_STATUS']) && $arRequest['INLINE_USER_STATUS'] == CBPTaskUserStatus::Cancel) {
            ...
            if (
                ...
                $label = $arTask["PARAMETERS"]["CommentLabelMessage"] <> '' ? $arTask["PARAMETERS"]["CommentLabelMessage"] : GetMessage(
                    "BPAA_ACT_COMMENT"
                );
    ...
    public function Subscribe(IBPActivityExternalEventListener $eventHandler)
    {
        ...
        if ($this->ApproveType == "any") {
            $this->WriteToTrackingService(
                str_replace("#VAL#", "{=user:".implode("}, {=user:", $arUsersTmp)."}", GetMessage("BPAA_ACT_TRACK1"))
            );
        } elseif ($this->ApproveType == "all") {
            $this->WriteToTrackingService(
                str_replace("#VAL#", "{=user:".implode("}, {=user:", $arUsersTmp)."}", GetMessage("BPAA_ACT_TRACK2"))
            );
        } elseif ($this->ApproveType == "vote") {
            $this->WriteToTrackingService(
                str_replace("#VAL#", "{=user:".implode("}, {=user:", $arUsersTmp)."}", GetMessage("BPAA_ACT_TRACK3"))
            );
        }
        ...
        $arParameters["TaskButton1Message"] = $this->IsPropertyExists(
            "TaskButton1Message"
        ) ? $this->TaskButton1Message : GetMessage("BPAA_ACT_BUTTON1");
        if ($arParameters["TaskButton1Message"] == '') {
            $arParameters["TaskButton1Message"] = GetMessage("BPAA_ACT_BUTTON1");
        }
        $arParameters["TaskButton2Message"] = $this->IsPropertyExists(
            "TaskButton2Message"
        ) ? $this->TaskButton2Message : GetMessage("BPAA_ACT_BUTTON2");
        if ($arParameters["TaskButton2Message"] == '') {
            $arParameters["TaskButton2Message"] = GetMessage("BPAA_ACT_BUTTON2");
        }
        $arParameters["TaskButton3Message"] = $this->IsPropertyExists(
            "TaskButton3Message"
        ) ? $this->TaskButton3Message : GetMessage("BPAA_ACT_BUTTON3");
        if ($arParameters["TaskButton3Message"] == '') {
            $arParameters["TaskButton3Message"] = GetMessage("BPAA_ACT_BUTTON3");
        }
        $arParameters["CommentLabelMessage"] = $this->IsPropertyExists(
            "CommentLabelMessage"
        ) ? $this->CommentLabelMessage : GetMessage("BPAA_ACT_COMMENT");
        ...
        $arParameters["AccessControl"] = $this->IsPropertyExists(
            "AccessControl"
        ) && $this->AccessControl == 'Y' ? 'Y' : 'N';
        ...
        if (!$this->IsPropertyExists("SetStatusMessage") || $this->SetStatusMessage == "Y") {
            ...
            $message = ($this->IsPropertyExists(
                    "StatusMessage"
                ) && $this->StatusMessage <> '') ? $this->StatusMessage : GetMessage("BPAA_ACT_INFO");
            $this->SetStatusTitle(
                str_replace(
     ...
     * (если остановка произошла во время выполнения данного действия).
     * @return int
     * @throws Exception
     */
    ...
    public function OnExternalEvent($arEventParameters = array())
    {
        ...
        if ($arUser = $dbUser->Fetch()) {
            $this->Comments = $this->Comments.
                CUser::FormatName(
                    ...
                    $arUser
                )." (".$arUser["LOGIN"]."): ".($approve ? GetMessage("BPAA_LOG_Y") : GetMessage("BPAA_LOG_N"))."\n".
                ($arEventParameters["COMMENT"] <> '' ? GetMessage(
                        "BPAA_LOG_COMMENTS"
        ...
        $rejecters = "";
        if (!$this->IsPropertyExists("SetStatusMessage") || $this->SetStatusMessage == "Y") {
            $messageTemplate = ($this->IsPropertyExists(
                    "StatusMessage"
                ) && $this->StatusMessage <> '') ? $this->StatusMessage : GetMessage("BPAA_ACT_INFO");
